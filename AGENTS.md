...
- Shared Expo UI form controls should live in `src/components/ui` wrappers that own the `Host` boundary, platform-specific native APIs, and any web fallback. Feature forms should compose those wrappers instead of repeating Expo UI native-boundary details inline.
